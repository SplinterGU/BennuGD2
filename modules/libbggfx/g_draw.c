...
int64_t draw_blend_mode = 0;

uint8_t draw_color_r = 0;
uint8_t draw_color_g = 0;
uint8_t draw_color_b = 0;
uint8_t draw_color_a = 0;

/* --------------------------------------------------------------------------- */

#define DRAW_PREPARE_RENDERER()    SDL_BlendMode blend_mode; \
    if ( gr_prepare_renderer( dest, clip, draw_blend_mode, &blend_mode ) ) return; \
    SDL_SetRenderDrawBlendMode( gRenderer, blend_mode ); \
    SDL_SetRenderDrawColor( gRenderer, draw_color_r, draw_color_g, draw_color_b, draw_color_a )
...
void draw_point( GRAPH * dest, REGION * clip, int64_t x, int64_t y, int64_t x2, int64_t y2 ) {
...
void draw_circle( GRAPH * dest, REGION * clip, int64_t x, int64_t y, int64_t r ) {

    int64_t cx = 0, cy = r;
    int64_t df = 1 - r, de = 3, dse = -2 * r + 5;

    SDL_Point * points = malloc( sizeof( SDL_Point ) * r * 8 );
    int64_t count = 0;
...
void draw_fcircle( GRAPH * dest, REGION * clip, int64_t x, int64_t y, int64_t r ) {
...
void draw_bezier( GRAPH * dest, REGION * clip, int64_t x1, int64_t y1, int64_t x2, int64_t y2, int64_t x3, int64_t y3, int64_t x4, int64_t y4, int64_t level ) {
    ...
    SDL_Point * points = malloc( sizeof( SDL_Point ) * n * 2 );
    if ( !points ) return;
    int64_t count = 0;
