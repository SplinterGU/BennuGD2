...
{
	c->data = (int64_t *) calloc(64, sizeof(int64_t));
	c->reserved = 64;

	c->loops = (int64_t *) calloc(16, sizeof(int64_t));
	c->loop_reserved = 8;
	c->loop_count = 1;
	c->loop_active = 0;

	c->labels = (int64_t *) calloc(16, sizeof(int64_t));
...
void codeblock_alloc(CODEBLOCK * c, int count) {
	c->reserved += count;
	c->data = (int64_t *) realloc(c->data, c->reserved * sizeof(int64_t));
	if (!c->data) {
		fprintf(stdout, "CODEBLOCK: out of memory\n");
		exit(1);
	}
...
static void codeblock_loop_alloc(CODEBLOCK * c, int count) {
	...
	c->loops = (int64_t *) realloc(c->loops, c->loop_reserved * sizeof(int64_t) * 2);
	if (!c->loops) {
		fprintf(stdout, "CODEBLOCK: out of memory\n");
		exit(1);
	}
...
static void codeblock_label_alloc(CODEBLOCK * c, int count) {
	...
	c->labels = (int64_t *) realloc(c->labels, c->label_reserved * sizeof(int64_t));
...
void codeblock_loop_start(CODEBLOCK * c, int loop, int begin) {
	if (c->loop_reserved <= loop) codeblock_loop_alloc(c, loop + 8 - c->loop_reserved);
	if (c->loop_count <= loop) c->loop_count = loop+1;
...
int codeblock_label_add(CODEBLOCK * c, int64_t identifier) {
...
void codeblock_label_set(CODEBLOCK * c, int64_t label, int offset) {
	c->labels[label] = offset;
}

int64_t codeblock_label_get(CODEBLOCK * c, int64_t label) {
	return c->labels[label];
}

int64_t codeblock_label_get_id_by_name(CODEBLOCK * c, int64_t identifier) {
    int i;
...
void codeblock_stepback(CODEBLOCK * c) {
	int64_t code  = c->data[c->previous];
	int param = 0;

	if (MN_PARAMS(code) > 0) param = c->data[c->previous+1];
...
void codeblock_add_block(CODEBLOCK * c, CODEBLOCK_POS from, CODEBLOCK_POS to) {
	if (c->current+(to.current - from.current)+2 >= c->reserved) codeblock_alloc(c,(to.current - from.current + 34) & 0x1F);

	memcpy(c->data + c->current, c->data + from.current, sizeof( int64_t ) * (to.current - from.current));
	c->current += to.current - from.current;
	c->previous = c->current -(to.current - to.previous);
	if (to.current - from.current > 2) c->previous2 = c->current -(to.current - to.previous2);
	else                               c->previous2 = c->previous;
}

void codeblock_add(CODEBLOCK * c, int64_t code, int64_t param) {
	if (!c) return;

	if (MN_PARAMS(code) == 0 && param) fprintf(stdout, "WARNING: mnemonic 0x%02" PRIX64 " don't get params\n", code);
