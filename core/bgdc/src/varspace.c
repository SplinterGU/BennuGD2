...
void varspace_dump( VARSPACE * n, int64_t indent ) {
    ...
    for ( i = 0; i < n->count; i++ ) {
        ...
        if ( typedef_is_array( n->vars[i].type ) ) {
            ...
            while ( typedef_is_array( r ) ) r = typedef_reduce( r );
        ...
        } else if ( typedef_is_struct( n->vars[i].type ) ) { /* Describe structs */
            printf( ":\n" );
...
void varspace_alloc( VARSPACE * n, int64_t count ) {
...
VARIABLE * varspace_search( VARSPACE * n, int64_t code ) {
    int i;
    for ( i = 0; i < n->count; i++ ) if ( n->vars[i].code == code ) return &n->vars[i];
    return 0;
}
